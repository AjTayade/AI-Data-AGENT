        ...
        if st.button("🪄 Run Exhaustive AI Sanitization", type="primary"):
            if not selected_files:
                ...
                for i, file in enumerate(selected_files):
                    ...
                    def check_needs_cleaning(df):
                        ...
                        for c in df.select_dtypes(include=['object']).columns:
                            ...
                            if sample.str.contains(r'^\s|\s$', regex=True).any():
                                issues.append(f"Hidden trailing/leading whitespace found in column '{c}'")
                                break
                            if sample.str.lower().isin(['na', 'n/a', '?', '-', 'none']).any():
                                issues.append(f"Junk null strings (e.g. 'n/a', '?') found in column '{c}'")
                                break
                        return issues
                    ...
                    if not issues_found:
                        st.success(f"✨ `{file}` passed all diagnostics! Skipping AI sanitization.")
                        clean_name = f"sanitized_{file}"
                        st.session_state['clean_cloud'][clean_name] = df_to_process
                        supabase.table('cleaned_datasets').insert({
                            'file_name': clean_name, 'data': df_to_process.replace({np.nan: None}).to_dict(orient='records'),
                            'user_id': user_id, 'notebook_name': nb_name
                        }).execute()
                        progress_bar.progress((i + 1) / total_files)
                        continue
                    ...
                    for col in df_to_process.columns:
                        ...
                        null_pct    = round(df_to_process[col].isnull().mean() * 100, 1)
                        sample_vals = df_to_process[col].dropna().astype(str).unique()[:5].tolist()
                        profile_lines.append(f"  - {col} | dtype={dtype} | nulls={null_pct}% | samples={sample_vals}")
                    col_profile = "\n".join(profile_lines)
...
with tab3:
    ...
    if not all_dash_data:
        ...
        if nb_name in st.session_state.get('dashboards', {}):
            ...
            # --- RENDER CHARTS WITH KEY FIX ---
            chart_titles = list(current_dash.keys())
            for row_start in range(0, len(chart_titles), 2):
                c1, c2 = st.columns(2)
                with c1:
                    t = chart_titles[row_start]
                    st.plotly_chart(current_dash[t], use_container_width=True, key=f"dash_chart_{nb_name}_{row_start}")
                if row_start + 1 < len(chart_titles):
                    with c2:
                        t = chart_titles[row_start + 1]
