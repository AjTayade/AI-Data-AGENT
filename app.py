...
if st.sidebar.button("🚪 Log Out", use_container_width=True):
    for _k in ['user', 'session', 'notebook_list', 'active_notebook',
               'raw_cloud', 'clean_cloud', 'dashboards', 'messages',
               '_session_restored', '_loaded_nb']:
        st.session_state.pop(_k, None)
    supabase.auth.sign_out()
    st.rerun()
...
# ── Notebook List: name button + 🗑 icon per row ──────────────
if st.session_state['notebook_list']:
    st.sidebar.divider()
    st.sidebar.markdown("**My Notebooks:**")
...
def confirm_delete_notebook():
    ...
    st.warning(
        f"Are you sure you want to permanently delete **'{nb_del}'**?\n\n"
        f"This will remove **all raw datasets, cleaned datasets, and dashboards** "
        f"stored in this notebook. This action **cannot be undone**."
    )
    col_yes, col_no = st.columns(2)
    with col_yes:
        if st.button("🗑️ Yes, Delete", use_container_width=True, type="primary"):
...
if _nb_changed or 'raw_cloud' not in st.session_state:
    ...
    with st.spinner(f"Loading {nb_name} vault..."):
        try:
            ...
            # Inflate saved dashboards from Supabase JSON → Plotly objects
            dash_res = supabase.table("saved_dashboards").select("dashboard_data") \
                .eq("user_id", user_id).eq("notebook_name", nb_name).execute()
            if dash_res.data:
                saved_json = dash_res.data[0]['dashboard_data']
                st.session_state['dashboards'][nb_name] = {
                    title: pio.from_json(fig_json)
                    for title, fig_json in saved_json.items()
                }
...
# ──────────────────────────────────────────────────────────────
# SHARED UTILITY — Smart Pre-Check Diagnostic
# ──────────────────────────────────────────────────────────────
def check_needs_cleaning(df: pd.DataFrame) -> list[str]:
    """
    Fast, native Python diagnostic — runs before any AI call.
    Returns a list of human-readable issue strings.
    An empty list means the file is already clean.
    """
    issues = []

    # Structural
    if df.isnull().all(axis=1).any():
        issues.append("Contains rows that are 100% empty")
    if df.isnull().all(axis=0).any():
        issues.append("Contains columns that are 100% empty")
    if df.duplicated().any():
        n_dups = int(df.duplicated().sum())
        issues.append(f"Contains {n_dups} exact duplicate row(s)")

    # Header hygiene
    bad_headers = [c for c in df.columns if not re.match(r'^[a-z0-9_]+$', str(c))]
    if bad_headers:
        issues.append(f"Headers not in strict snake_case — e.g. {bad_headers[:3]}")

    # Per-column quick sample checks (capped at 200 rows for speed)
    for col in df.select_dtypes(include=['object']).columns:
        sample = df[col].dropna().astype(str).head(200)

        if sample.str.contains(r'^\s|\s$', regex=True).any():
            issues.append(f"Leading/trailing whitespace in column '{col}'")
            break  # one instance is enough to trigger the AI

    # Junk null check across all object columns
    junk_nulls = {'na', 'n/a', '?', '-', '--', 'none', 'null', 'nil',
                  'nan', 'unknown', 'undefined', 'missing', '#n/a', '#null!'}
    for col in df.select_dtypes(include=['object']).columns:
        sample = df[col].dropna().astype(str).str.lower().str.strip().head(200)
        if sample.isin(junk_nulls).any():
            issues.append(f"Junk null strings (e.g. 'n/a', '?') in column '{col}'")
            break

    # Missing value check across all columns
    high_null_cols = [
        c for c in df.columns
        if df[c].isnull().mean() > 0.0
    ]
    if high_null_cols:
        issues.append(
            f"{len(high_null_cols)} column(s) contain missing values — "
            f"e.g. {high_null_cols[:3]}"
        )

    # Numeric-looking strings check
    for col in df.select_dtypes(include=['object']).columns:
        sample = df[col].dropna().astype(str).head(100)
        numeric_like = sample.str.replace(r'[\$€£¥,%,]', '', regex=True)
        try:
            parsed = pd.to_numeric(numeric_like, errors='coerce')
            if parsed.notna().mean() > 0.8:
                issues.append(
                    f"Column '{col}' looks numeric but is stored as text "
                    f"(may contain currency/% symbols)"
                )
                break
        except Exception:
            pass
        ...
        if not uploaded_files:
            ...
            for file in uploaded_files:
                ...
                if file.name in st.session_state.get('raw_cloud', {}):
                    st.warning(
                        f"⚠️ **'{file.name}'** already exists in this notebook. "
                        f"Skipped. Delete it in the file manager below to replace it."
                    )
                    continue
                ...
                try:
                    ...
                    if existing_res.data:
                        ...
                        st.session_state['raw_cloud'][file.name] = reused_df
                        st.info(
                            f"♻️ **'{file.name}'** found in notebook **'{source_nb}'** — "
                            f"linked here. No duplicate data stored."
                        )
                        continue
                except Exception:
                    pass  # fall through to normal upload
        ...
        if st.button("🪄 Run Exhaustive AI Sanitization", type="primary"):
            if not selected_files:
                ...
                for i, file in enumerate(selected_files):
                    ...
                    # ══ STEP 1: SMART PRE-CHECK DIAGNOSTIC ════════════
                    issues_found = check_needs_cleaning(df_to_process)

                    # ── FAST PATH: file is already clean ──────────────
                    if not issues_found:
                        st.success(
                            f"✨ **`{file}`** passed all diagnostics — already perfectly clean! "
                            f"Skipping AI sanitization."
                        )
                        clean_name = f"sanitized_{file}"
                        st.session_state['clean_cloud'][clean_name] = df_to_process
                        supabase.table('cleaned_datasets').insert({
                            'file_name': clean_name,
                            'data':      df_to_process.replace({np.nan: None}).to_dict(orient='records'),
                            'user_id':   user_id, 'notebook_name': nb_name
                        }).execute()
                        progress_bar.progress((i + 1) / total_files)
                        continue  # skip AI entirely for this file

                    # ── Show the user exactly what was found ──────────
                    issues_display = "\n".join([f"🔴 {iss}" for iss in issues_found])
                    st.info(
                        f"🔬 **Diagnostic complete for `{file}`** — "
                        f"{len(issues_found)} issue(s) found. AI is targeting them now:\n\n"
                        f"{issues_display}"
                    )

                    # ══ STEP 2: RICH COLUMN PROFILE ═══════════════════
                    profile_lines = []
                    for col in df_to_process.columns:
                        dtype       = str(df_to_process[col].dtype)
                        null_pct    = round(df_to_process[col].isnull().mean() * 100, 1)
                    ...
                    # ══ STEP 3: TARGETED DYNAMIC PROMPT ══════════════
                    # The prompt is built from the actual diagnostic output —
                    # the AI only fixes what is genuinely broken.
                    clean_prompt = f"""
You are an elite Senior Data Engineer. Write a Python function `clean_data(df)` to sanitize this dataset.

DATASET PROFILE (column-level intelligence):
{col_profile}

SAMPLE DATA (first 10 rows):
{full_sample}

🎯 TARGETED DIAGNOSTICS — PRIORITY FIXES (detected by a native Python pre-scan):
The following specific issues MUST be explicitly addressed in your code:
{issues_display}

GENERAL HYGIENE (apply where relevant, skip what is already clean):
1. Convert all column headers to strict snake_case (lowercase, underscores, no special chars).
2. For every object/string column: strip leading/trailing whitespace via df[col].str.strip().
3. Normalize multiple internal spaces to single space.
4. Replace all junk null strings with np.nan (case-insensitive):
   '', 'n/a', 'na', 'none', 'null', 'nil', 'nan', '-', '--', '?', 'unknown', '#n/a'.
5. Numeric columns with null rate <20%: impute with column median.
   20–50%: leave as NaN. >50%: drop the column.
6. Date/time columns (by name keyword or pattern): convert with pd.to_datetime(..., errors='coerce')
   then format to ISO 8601: df[col].dt.strftime('%Y-%m-%d'). Leave parse failures as NaN.
7. Numeric-looking string columns (with $, €, %, commas): strip symbols and cast to float.
8. Low-cardinality string columns (<30 unique): normalise casing variants to most-frequent form.
9. Outlier capping (Winsorize): cap values below Q1-1.5*IQR and above Q3+1.5*IQR for numeric
   columns where outlier count exceeds 1% of rows. Never drop rows.
10. Boolean-like columns (values in yes/no/true/false/1/0/y/n): map to Python bool.
11. ID/code/zip/phone columns: keep as string, never cast numeric, never strip leading zeros.
12. After all cleaning: if df.empty: raise ValueError("Cleaning resulted in empty DataFrame")

STRICT CODE RULES — THESE WILL CAUSE RUNTIME ERRORS IF VIOLATED:
- CRITICAL: Before applying ANY .str method, always guard with:
  `if df[col].dtype == object: df[col] = df[col].astype(str).str...`
  NEVER call .str methods on float or int columns.
- Use df[col].str methods for per-column string ops.
  NEVER use df.applymap(), NEVER use df.map() on the whole DataFrame.
- Do NOT aggregate, pivot, merge, or reshape the DataFrame.
- Imports INSIDE the function body: import pandas as pd, import numpy as np, import re
- The function signature must be exactly: def clean_data(df):
- Return ONLY the cleaned DataFrame. NOTHING else.
- Return ONLY valid, executable Python code.
  NO markdown fences. NO backticks. NO inline explanations.
"""
                    ...
                    try:
                        ...
                        st.session_state['clean_cloud'][clean_name] = df_cleaned
                        st.success(f"✅ Auto-saved as **'{clean_name}'** to database.")
                ...
                status_text.success(f"🎉 All {total_files} file(s) processed and saved!")
...
with tab3:
    ...
    if not all_dash_data:
        ...
        # ── PERSISTENT DASHBOARD: already generated & saved ───
        if nb_name in st.session_state.get('dashboards', {}):
            st.success("✅ Persistent dashboard loaded from vault.")
            current_dash = st.session_state['dashboards'][nb_name]

            # ── Export to HTML ─────────────────────────────────
            with st.expander("📥 Export Dashboard as HTML Report", expanded=False):
                st.caption(
                    "Download this dashboard as a standalone, interactive HTML file "
                    "you can share with stakeholders — no Streamlit required."
                )
                html_content = (
                    f"<html><head><title>Executive Report: {nb_name}</title>"
                    f"<style>body{{font-family:'Segoe UI',sans-serif;background:#f4f6f9;"
                    f"padding:40px;color:#1a1a2e}}.container{{max-width:1200px;margin:auto}}"
                    f"h1{{border-bottom:2px solid #e2e8f0;padding-bottom:10px;color:#2c3e50}}"
                    f".chart-box{{background:white;margin-bottom:30px;padding:25px;"
                    f"border-radius:8px;box-shadow:0 4px 6px rgba(0,0,0,.05)}}"
                    f"</style></head><body><div class='container'>"
                    f"<h1>Executive Report: {nb_name}</h1>"
                )
                for title, fig in current_dash.items():
                    html_content += (
                        f"<div class='chart-box'><h2>{title}</h2>"
                        f"{fig.to_html(full_html=False, include_plotlyjs='cdn')}</div>"
                    )
                html_content += "</div></body></html>"
                ...
                st.download_button(
                    ...
                    data=html_content,
                    file_name=f"Executive_Report_{nb_name}.html",
                    mime="text/html",
                    use_container_width=True
                )

            # ── Render charts in 2-col grid ────────────────────
            chart_titles = list(current_dash.keys())
            for row_start in range(0, len(chart_titles), 2):
                c1, c2 = st.columns(2)
                with c1:
            ...
            # ── Regenerate option ──────────────────────────────
            st.divider()
            if st.button("🔄 Regenerate Dashboard", type="secondary"):
                supabase.table("saved_dashboards").delete() \
                    .eq("user_id", user_id).eq("notebook_name", nb_name).execute()
                del st.session_state['dashboards'][nb_name]
                st.rerun()

        # ── DASHBOARD BUILDER (no saved dashboard yet) ─────────
        else:
            st.caption(
                "Select one or more datasets — cleaned datasets recommended. "
                "The AI analyses column types, value distributions, and cross-file "
                "relationships to produce a story-driven, presentation-ready dashboard."
            )
            ...
            if dash_files:
                ...
                # ── Deep schema + stats for the prompt ────────
                ...
                if st.button("🪄 Generate Executive Dashboard", type="primary"):
                    with st.spinner("AI is analysing data and designing your dashboard..."):
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CHART SELECTION — choose based on actual column types above:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- NUMERIC vs NUMERIC       → scatter with trendline='ols'
- CATEGORICAL vs NUMERIC   → horizontal sorted bar, orientation='h'
- DATETIME vs NUMERIC      → line chart, markers=True
- SINGLE CATEGORY dist.    → pie if <10 unique vals, else bar of value_counts
- NUMERIC distribution     → histogram with marginal='box', nbins=30
- MULTIPLE NUMERIC cols    → correlation heatmap via go.Heatmap on df.corr().round(2)
- MULTIPLE CATEGORIES      → grouped bar, barmode='group'
- CROSS-FILE (if join possible) → merge on common key, then comparison chart
Do NOT repeat the same Plotly function more than twice.
Aim for: 1 Trend, 1 Distribution, 1 Comparison, 1 Relationship, 2 others.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
EXECUTIVE DESIGN RULES — EVERY chart must follow ALL of these:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. STORY TITLE: Titles must describe the insight, not the chart type.
   Good: "Revenue Peaked in Q4 — Up 34% YoY"
   Bad:  "Bar Chart" or "Sales vs Time"
2. TEMPLATE: Apply template="plotly_white" to every figure.
3. CLEAN LAYOUT: fig.update_yaxes(showgrid=False). Remove top/right borders.
4. COLORS: px.colors.qualitative.Prism for categorical; px.colors.sequential.Blues for sequential.
5. ANNOTATIONS: text_auto='.2s' on bar charts. Add mean/median reference lines with fig.add_vline().
6. HOVER: Set hover_data to include all relevant columns.
7. update_layout() on EVERY figure:
   font=dict(family="Inter, sans-serif", size=12, color="#1a1a2e"),
   plot_bgcolor="#f8f9fa", paper_bgcolor="white",
   margin=dict(l=50, r=30, t=70, b=50),
   title_font=dict(size=15, color="#1a1a2e"),
   legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1)
8. Bar charts: textposition='outside'. Line charts: line=dict(width=2).
9. Heatmap: annotate cells, colorscale='RdBu_r', zmid=0.
10. Scatter: if a third numeric col exists, set size=third_col_name.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
FUNCTION CONTRACT:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- Extract DataFrames: df = data_dict["exact_key_name_from_schema_above"]
...
- Wrap EACH chart in its own try/except — if it fails, generate a fallback:
    except Exception:
        fig = go.Figure()
        fig.add_annotation(text="Chart unavailable for this data configuration",
                           xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False,
                           font=dict(size=14, color="grey"))
        fig.update_layout(title="Data Overview", template="plotly_white")
- Return exactly:
  {{ "Story Title 1": fig1, "Story Title 2": fig2, "Story Title 3": fig3,
     "Story Title 4": fig4, "Story Title 5": fig5, "Story Title 6": fig6 }}

STRICT CODE RULES:
- Imports inside function: import plotly.express as px; import plotly.graph_objects as go
- Do NOT import pandas or numpy (already in scope as pd and np).
- Do NOT use df.applymap() — use df.map() or df[col].apply() instead.
- NEVER hardcode column names — always infer from dtypes at runtime.
- Return ONLY valid executable Python. NO markdown. NO backticks. NO explanations.
"""
                        try:
                            dash_response = model.generate_content(dash_prompt)
                            ...
                            local_vars   = {}
                            safe_globals = globals().copy()
                            safe_globals.update({'px': px, 'go': go, 'pd': pd, 'np': np})
                            exec(dash_code, safe_globals, local_vars)
                            dashboard_figs = local_vars['build_dashboard'](df_dict)
                            ...
                            # ── Save to memory and reload to render ────
                            st.session_state.setdefault('dashboards', {})[nb_name] = dashboard_figs
                            st.rerun()
